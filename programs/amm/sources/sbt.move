...
//0x6c6754eaea4bf803f071222e5eacf46cfa7a000c
    struct SBT has key{
        id: UID,
        url: Url
    }
    ...
    public entry fun update_svg_name(sbt: &mut SBT, name: vector<u8>){
    ...
    fun test_sbt(){
        ...
        {
            debug::print(&sbt);
            update_url(&mut sbt, b"123");
        };

        let foo = b"123";
        let res = b"data:image/svg+xml;base64,123";

        let foo = prefix_svg(foo);

        assert!(foo==res, 1);

        let svg = generate_svg(b"fsd");
        update_url(&mut sbt, prefix_svg(svg));
